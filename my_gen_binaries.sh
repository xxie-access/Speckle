...
if [ -z "$RUN" ]; then
    RUN="spike -t2 --ic=32:8:64 --dc=32:8:64 --l2=512:16:64 --tt=64:8:64:0 pk -c "
fi
CMD_FILE=commands.txt
INPUT_TYPE=train

if [ -z "$TAGGER_EXEC" ]; then
   echo " Please set the TAGGER_EXEC environment variable to point to auto-tagger executable."
   exit 1
fi
...
#   cd $SPEC_DIR; . ./shrc; time runspec --config ${CONFIG} --size ${INPUT_TYPE} --action scrub int
   ...
   for b in ${BENCHMARKS[@]}; do
      ...
      #rename and tag benchmark executables
      mv $BMK_DIR/${SHORT_EXE}_base.${CONFIG} $BMK_DIR/${SHORT_EXE}_base_untagged.${CONFIG}
      eval ${TAGGER} $BMK_DIR/${SHORT_EXE}_base_untagged.${CONFIG} $BMK_DIR/${SHORT_EXE}_base.${CONFIG}
